...
//		music -= old_login_music
	...
	if(world.params["bt_compare"] || world.GetConfig("env", "BT_COMPARE") || world.params["faction_test"] || world.GetConfig("env", "FACTION_TEST") || world.GetConfig("env", "VN_PROFILE"))
...
/datum/controller/subsystem/ticker/proc/create_characters()
	...
	for(var/i in GLOB.new_player_list)
		var/mob/dead/new_player/player = i
		if(!player)
			message_admins("THERES A FUCKING NULL IN THE NEW_PLAYER_LIST, REPORT IT TO SCARLET REACH DEVELOPMENT STAFF NOW!")
			continue
		if(!player.mind)
			message_admins("THERES A MIND LACKING PLAYER IN THE NEW_PLAYER_LIST, REPORT IT TO SCARLET REACH DEVELOPMENT STAFF NOW!")
			continue
		if(player.ready == PLAYER_READY_TO_PLAY)
