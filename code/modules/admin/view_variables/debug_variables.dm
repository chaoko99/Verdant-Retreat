	...
	if(D)
		if(islist(D))
			...
			if(value)
				...
				value = L[name]
			header = "<li style='backgroundColor:white'>([VV_HREF_TARGET_1V(L, VV_HK_LIST_EDIT, "E", index)]) ([VV_HREF_TARGET_1V(L, VV_HK_LIST_CHANGE, "C", index)]) ([VV_HREF_TARGET_1V(L, VV_HK_LIST_REMOVE, "-", index)]) "
		else
